...
# ============ JAVASCRIPT ============
JS_CODE = r"""
const C1='#0f172a',C2='#334155',C3='#64748b',C4='#94a3b8',C5='#cbd5e1',GRN='#059669',GOLD='#b8860b';
const PAL=[C1,C2,C3,C4,C5,GRN,'#6366f1','#ec4899','#f59e0b','#14b8a6','#8b5cf6','#ef4444'];
...
function getYears(){
  var all=[];
  ['loans_customer','payments','dep_segments','remit_channel','int_rates_loans'].forEach(function(k){
    (D[k]||[]).forEach(function(r){var y=parseInt(pk(r.period));if(y>1990&&all.indexOf(y)<0)all.push(y)});
  });return all.sort();
}
...
function flt(arr){
  var yf=parseInt(document.getElementById('f-yf').value)||2020;
  var yt=parseInt(document.getElementById('f-yt').value)||2026;
  var mo=document.getElementById('f-mo').value;
  return arr.filter(function(r){var p=pk(r.period);var y=parseInt(p);var m=p.split('-')[1];
    if(y<yf||y>yt)return false;if(mo!=='all'&&m!==mo)return false;return true});
}
...
function prior12(arr,latP,fn){
  ...
  var ty=y-1,tm=m<10?'0'+m:String(m);
...
function lc(id,lbs,ds,o){return mc(id,'line',{data:{labels:lbs.map(fmtP),datasets:ds},options:Object.assign({responsive:true,aspectRatio:2,interaction:{mode:'index',intersect:false},plugins:{legend:{position:'top',labels:{font:{size:10}}},tooltip:ttMoney},scales:{x:{ticks:{maxTicksAutoSkip:true,maxRotation:0,font:{size:9}}},y:{beginAtZero:false,ticks:{font:{size:10},callback:function(v){return fmtMoney(v)}}}}},o||{})})}
function lcPct(id,lbs,ds,o){return mc(id,'line',{data:{labels:lbs.map(fmtP),datasets:ds},options:Object.assign({responsive:true,aspectRatio:2,interaction:{mode:'index',intersect:false},plugins:{legend:{position:'top',labels:{font:{size:10}}},tooltip:ttPct},scales:{x:{ticks:{maxTicksAutoSkip:true,maxRotation:0,font:{size:9}}},y:{ticks:{font:{size:10},callback:function(v){return v.toFixed(1)+'%'}}}}},o||{})})}
function bc(id,lbs,d,c,o){return mc(id,'bar',{data:{labels:lbs,datasets:[{data:d,backgroundColor:c||PAL,borderRadius:3,maxBarThickness:32}]},options:Object.assign({responsive:true,aspectRatio:1.5,indexAxis:'y',plugins:{legend:{display:false},tooltip:ttMoney},scales:{x:{ticks:{font:{size:10},callback:function(v){return fmtMoney(v)}}},y:{ticks:{font:{size:10}}}}},o||{})})}
...
function dualAxis(id,lbs,ds1,ds2,l1,l2){return mc(id,'line',{data:{labels:lbs.map(fmtP),datasets:[
  {label:l1,data:ds1,borderColor:C1,backgroundColor:C1+'15',fill:true,tension:.3,yAxisID:'y'},
  {label:l2,data:ds2,borderColor:C3,borderDash:[4,2],tension:.3,yAxisID:'y1'}
...
if(custPeriods.length){
  showCanvas('ch-ln-cust');lc('ch-ln-cust',custPeriods,custSegs.map(function(seg,i){
    ...
    return{label:seg.replace('Non-Financial ','NF '),data:custPeriods.map(function(p){return pm[p]||null}),borderColor:PAL[i],tension:.3,spanGaps:true,borderWidth:i<2?2:1};
  }));
...
if(matHH.length){showCanvas('ch-ln-mat');bc('ch-ln-mat',matHH.map(function(x){return x.l}),matHH.map(function(x){return x.v}),PAL)}
else noData('ch-ln-mat','No HH maturity data');
...
if(matNFC.length){showCanvas('ch-ln-matc');bc('ch-ln-matc',matNFC.map(function(x){return x.l}),matNFC.map(function(x){return x.v}),PAL.slice(3))}
else noData('ch-ln-matc','No NFC maturity data');
...
if(sortedSectors.length){showCanvas('ch-ln-sec');bc('ch-ln-sec',sortedSectors.map(function(s){return s.sector}),sortedSectors.map(function(s){return s.value}),PAL)}
...
if(dsPeriods.length){
  ...
  showCanvas('ch-dp-share');lcPct('ch-dp-share',dsPeriods,[
    ...
    {label:'NFC',data:dsPeriods.map(function(p){return totPm[p]?nfcPm[p]/totPm[p]*100:null}),borderColor:C3,tension:.3}
  ]);
...
if(rcoB.length){showCanvas('ch-rm-co');bc('ch-rm-co',rcoB.map(function(r){return r.country}),rcoB.map(function(r){return r.value}),PAL,{plugins:{tooltip:ttPct},scales:{x:{title:{display:true,text:'% share'},ticks:{callback:function(v){return v.toFixed(1)+'%'}}}}})}
...
out_path = os.path.join(BASE, 'Kosovo_Banking_Intelligence_v4.html')
